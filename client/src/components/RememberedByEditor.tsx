...
export function RememberedByEditor({ trait, onChange, accentColor = "#b45353", accentSecondaryColor = accentColor, effectsLocked = false }: { trait: Trait; onChange: (trait: Trait) => void; accentColor?: string; accentSecondaryColor?: string; effectsLocked?: boolean }) {
    ...
    <Dialog>
      ...
      >
        ...
        <div className="space-y-3 pt-3">
          ...
          <Textarea value={trait.effect} onChange={(event) => onChange({ ...trait, effect: event.target.value })} placeholder="Effect" className="min-h-[180px] resize-y bg-black/50" />
          <Textarea value={trait.description} onChange={(event) => onChange({ ...trait, description: event.target.value })} placeholder="Description" className="min-h-[180px] resize-y bg-black/50" />
            ...
            {people.map((person, index) => (
              <div key={index} className="space-y-3 rounded-lg border border-white/5 bg-black/20 p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <Input value={person.effect} disabled={effectsLocked} onChange={(event) => updatePerson(index, { effect: event.target.value })} placeholder="Known effect or connection" className={`bg-black/50 ${effectsLocked ? "pointer-events-none select-none blur-[6px] opacity-40" : ""}`} aria-label={effectsLocked ? "Effect currently hidden and locked" : "Known effect or connection"} />
                </div>
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                  ...
                  <label className={`flex items-center gap-2 text-xs text-muted-foreground ${effectsLocked ? "pointer-events-none select-none blur-[5px] opacity-40" : ""}`}><Switch checked={person.effectKnown} disabled={effectsLocked} onCheckedChange={(checked) => updatePerson(index, { effectKnown: checked })} /> Effect discovered</label>
          ...
          {adding ? (
            <div className="space-y-3 rounded-lg border border-fuchsia-400/20 bg-fuchsia-500/5 p-4">
              ...
              <Input value={draft.effect} disabled={effectsLocked} onChange={(event) => setDraft({ ...draft, effect: event.target.value })} placeholder="Effect or connection" className={`bg-black/50 ${effectsLocked ? "pointer-events-none select-none blur-[6px] opacity-40" : ""}`} aria-label={effectsLocked ? "Effect currently hidden and locked" : "Effect or connection"} />
              <div className="flex flex-wrap gap-5">
                ...
                <label className={`flex items-center gap-2 text-xs text-muted-foreground ${effectsLocked ? "pointer-events-none select-none blur-[5px] opacity-40" : ""}`}><Switch checked={draft.effectKnown} disabled={effectsLocked} onCheckedChange={(effectKnown) => setDraft({ ...draft, effectKnown })} /> Effect discovered</label>
              </div>
